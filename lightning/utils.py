...
################### summarize function ########################
def summarize(model, ## Lightning Model
              dataloader, 
              num_beams,
              device):
    ...
    with torch.no_grad():
        for step, data in bar:
            ...
            decoded_preds = model.generate_text(ids, masks, eval_beams = num_beams, min_len = 30, max_len = 70)
...
################# rouge_metric for mecab #################
# from konlpy.tag import Mecab
# source: https://github.com/seujung/KoBART-summarization/blob/main/rouge_metric.py
class Rouge:
